...
export const RouletteGame = ({ 
  ...
  const handleSpin = async (type: 'color' | 'number', selection: string | number) => {
    ...
    // 1. DÉBITO VISUAL IMEDIATO (Optimistic UI)
    setIsSpinning(true);
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('play-roulette', {
        body: { betType: type, selection, betAmount, user_id: user.id }
      });

      if (error) throw error;
      
      // Lógica de Giro Visual
      const winningIndex = WHEEL_ORDER.indexOf(data.resultNumber);
      ...
      if (data.payout > 0) {
        setWinData({ amount: data.payout });
      } else {
        toast({ title: "Derrota", description: "Tente novamente!", variant: "default" });
      }
      
      // 2. SINCRONIZAÇÃO COM O BACKEND
      // Usa o valor exato retornado pelo banco de dados
      ...
      console.error(e);
      // Rollback visual em caso de erro (devolve o dinheiro)
      setBalance((prev: number) => prev + betAmount);
        ...
        >
          <img 
            ...
            onError={(e) => {
              ...
              e.currentTarget.parentElement!.style.background = 'radial-gradient(circle, #222, #000)';
            }}
          />
